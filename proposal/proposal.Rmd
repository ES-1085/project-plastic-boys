...
```{r prelim-plot-gi-bar, fig.alt="This bar graph shows the distribution of microplastics within the fish. We can see that most of the microplastics can be found in the GI tract."}
fish_microplastics %>%
  group_by(fish_id) %>%
...
```
```{r prelim-plot-quant-density, fig.alt="In this ridgeline it compares the individual fish and the count of microplastics within. As we can see most of the fish have similar overall counts."}
fish_microplastics %>%
  group_by(fish_id) %>%
  filter(quantity < 60) %>%
